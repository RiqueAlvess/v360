...
class TaskQueue(Base, TimestampMixin):
    ...
    tipo: Mapped[TaskQueueType] = mapped_column(
        sa.Enum(TaskQueueType, name="task_queue_type", create_type=False, values_callable=lambda obj: [e.value for e in obj]),
        nullable=False,
    )
    payload: Mapped[dict[str, Any]] = mapped_column(JSONB, nullable=False)
    status: Mapped[TaskQueueStatus] = mapped_column(
        sa.Enum(TaskQueueStatus, name="task_queue_status", create_type=False, values_callable=lambda obj: [e.value for e in obj]),
        nullable=False,
